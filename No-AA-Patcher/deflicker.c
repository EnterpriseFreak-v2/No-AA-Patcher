...
void applyDeflickerPatch(char* buffer, size_t bufsize)
{
	...
	for (int i = 0; i < bufsize - sizeof(ptr_new_nofilt); i++)
	{
		...
		if (memcmp(position, ptr_old_progSf, 7) == 0)
		{
			memcpy(position, ptr_new_nofilt, 7);
		}
		else if (memcmp(position, ptr_old_progAa, 7) == 0)
		{
			memcpy(position, ptr_new_nofilt, 7);
		}
		else if (memcmp(position, ptr_old_mkwii1, 7) == 0)
		{
			memcpy(position, ptr_new_nofilt, 7);
		}
		else if (memcmp(position, ptr_old_mkwii2, 7) == 0)
		{
			memcpy(position, ptr_new_nofilt, 7);
		}
		else if (memcmp(position, ptr_old_galaxy, 7) == 0)
		{
			memcpy(position, ptr_new_nofilt, 7);
		}
	}
